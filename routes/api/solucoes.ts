...
router.post("/login", wrap(async (req: express.Request, res: express.Response) => {
	let u = await .cookie(req);
	jsonRes(res, 403, u ? null : await Solucoes.efetuarLogin(req.body.login as string, req.body.senha as string, res)[0]);
}));
...
router.post("/alterarPerfil", wrap(async (req: express.Request, res: express.Response) => {
    let u = await Solucoes.cookie(req, res);
	if (!u)
		return;
...
router.get("/listar", wrap(async (req: express.Request, res: express.Response) => {
    let u = await Solucoes.cookie(req, res, true);
	if (!u)
		return;
    res.json(await Solucoes.listar());
}));
...
router.get("/obter", wrap(async (req: express.Request, res: express.Response) => {
    ...
    res.json(isNaN(id) ? null : await Solucoes.obter(id));
}));
...
router.post("/criar", wrap(async (req: express.Request, res: express.Response) => {
	...
	if (!u)
		return;
    u = req.body as Solucoes;
	u.perfil = parseInt(req.body.perfil);
    jsonRes(res, 400, u ? await Solucoes.criar(u) : "Dados inválidos");
}));
